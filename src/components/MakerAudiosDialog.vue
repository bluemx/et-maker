<template>
        ...
        <div v-for="(item, index) in voices" :key="item.id">
            <div class="border-1 border-slate-800 bg-slate-90 p-0.5 bg-slate-800/20">
                <div class="flex gap-1 items-center">
                    ...
                    <VoiceView sample :item="item.voice"></VoiceView>
    ...
    <div class="p-1 border-1 border-amber-500">
      <MakerAudiosDialogInput ref="DialogInput" v-if="voices.length>0" :voices="voices" :symL="symL" :symC="symC" :symD="symD"></MakerAudiosDialogInput>
    </div>
        ...
        <template v-for="(item, index) in DialogInput.dialogs" v-if="DialogInput && DialogInput.dialogs">
            <div class="border-1 border-slate-800 bg-slate-90 p-0.5 bg-slate-800/20 p-1">
                <div class="flex gap-2 items-center">
                        ...
                        <div class="ml-auto">
                            <AudioGenerator :filename="fileprefixname?fileprefixname+'-'+index : item.id" :item="item"></AudioGenerator>
                        </div>
...
import { useSortable } from '@vueuse/integrations/useSortable'
    ...
    const fileprefixname = useStorage('etmaker-voicesdialog-filename')
    ...
    const voices = useStorage('etmaker-voicesDialog', []);
    ...
    const addvoice = (item) => {
        ...
        let exists = voices.value.some((i) => i.voice.value === item.value);
